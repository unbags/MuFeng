...
public class KnowledgeIngestionService {
    ...
    public void clear() {
        try {
            ...
            if (nativeClient instanceof MilvusServiceClient) {
                MilvusServiceClient milvusClient = (MilvusServiceClient) nativeClient;
                milvusClient.dropCollection(DropCollectionParam.newBuilder()
                    .withCollectionName("mufeng_kb")
                    .build());
                log.info("Dropped existing knowledge base collection");

                if (vectorStore instanceof InitializingBean) {
                    InitializingBean initBean = (InitializingBean) vectorStore;
