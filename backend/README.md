...
| MySQL | 8.0.33 | 关系型数据库 |
| Redis | 6+ | 缓存与会话管理 |
| Spring Security | 2.7 | 认证与授权 |
| JWT | 0.9.1 | 无状态令牌认证 |
| WebSocket | 2.7 | 实时推送（订单状态通知等） |
...
│   ├── config/          # Spring 配置类（Security、CORS、WebSocket 等）
│   ├── controller/      # 控制器层
│   ├── service/         # 业务逻辑层
...
确保 Redis 服务已启动在 `localhost:6379`（默认端口，无需密码）。
...
spring:
  datasource:
    ...
    username: ${DB_USERNAME:root}      # 可通过环境变量 DB_USERNAME 覆盖
    password: ${DB_PASSWORD:123456}    # 可通过环境变量 DB_PASSWORD 覆盖
  redis:
    host: ${REDIS_HOST:localhost}
    port: ${REDIS_PORT:6379}
    password: ${REDIS_PASSWORD:}       # 生产环境建议设置密码
```
...
| Profile | SQL 初始化 | 数据库连接池 | 适用场景 |
|---------|-----------|-------------|----------|
| `dev` | 不执行（手动管理） | 默认配置 | 本地开发 |
| `prod` | 禁用 | HikariCP 增强配置（最大 200 连接） | 生产部署 |
...
# 3. 后台运行（推荐使用 systemd 或 supervisor）
